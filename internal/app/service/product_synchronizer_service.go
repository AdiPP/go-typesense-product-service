		...
		_, err = p.typesenseRepo.UpsertProductSku(
			&typesense.UpsertProductSkuRequest{
				Document: typesense.ProductSkuDocument{
					...
					ProductImageNoBackground:    productImage.ProductImageNoBackground,
					ProductImageNoBackgroundURL: fmt.Sprintf("%s/%s", os.Getenv("CDN_IMAGE_URL"), productImage.ProductImageNoBackground),
...
func NewProductSynchronizerService(pgsqlDatabase *pgsql.Repository) *ProductSynchronizerService {
	return &ProductSynchronizerService{pgsqlRepo: pgsqlDatabase}
}
