...
func (r *Repository) FindAllProductImagesByProductIds(productIds []int64) (response FindAllProductImagesByProductIdsResponse, err error) {
	response = FindAllProductImagesByProductIdsResponse{
		ProductImages: FindAllProductImagesByProductIdsResponseProductImages{},
	}

	ds := r.database.
		From(goqu.L(fmt.Sprintf("(%s)", findAllProductImagesByProductIdsQuery)).As("d")).
