...
select
	...
	rp.status_record
from rns_product rp
join rns_uom ru on rp.uom_id = ru.uom_id
join rns_product_preorder_type rppt on rp.product_preorder_type_id = rppt.product_preorder_type_id
join rns_product_condition rpc on rp.product_condition_id = rpc.product_condition_id
join rns_store rs on rp.store_id = rs.id
join store.rns_store_status rss on rs.store_status_id = rss.store_status_id
join rns_category rc on rc.category_id = rp.category_id
where
	ru.status_record <> 'D'
...
var findAllProductSkusByProductIdsQuery = `
select
	rps.is_default,
	rps.product_id,
	rps.product_price,
...
from
	rns_product_sku rps
join rns_product_status rps2 on rps2.product_status_id = rps.product_status_id
where
	rps.status_record <> 'D'
`
...
var findAllProductImagesByProductIdsQuery = `
select 
    rpi.product_id,
    rpi.product_sku_id,
    rpi.image_type_id,
...
join rns_product_sku rps on rpi.product_sku_id = rps.product_sku_id
